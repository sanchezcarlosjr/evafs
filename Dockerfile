...
# and `pip install evafs` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "evafs", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n evafs -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "evafs", "/bin/bash", "-c"]
...
ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "evafs", "evafs web"]
